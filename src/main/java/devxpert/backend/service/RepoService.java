...
import devxpert.backend.dto.IndexStatusResponse;
import devxpert.backend.dto.RepositoryResponse;
import devxpert.backend.entity.Repository;
import devxpert.backend.entity.User;
import devxpert.backend.exception.NotFoundException;
import devxpert.backend.repository.RepositoryRepository;
import devxpert.backend.service.github.GithubApiClient;
    ...
    public List<RepositoryResponse> syncAndListRepos(UUID userId) {
        ...
        for (Map<String, Object> remote : remoteRepos) {
            ...
            repo.setGithubRepoId(githubRepoId);
                ...
                String owner = ownerObj instanceof Map<?, ?> ownerMap && ownerMap.get("login") != null
                    ? String.valueOf(ownerMap.get("login"))
                    : String.valueOf(ownerObj);
                repo.setOwner(parts.length > 1 ? parts[0] : owner);
